...
JUDGE_TYPE BPM180::Judge(int itr, float spendTime)
{
	//v4.3 added
...
BPM180::BPM180()
{
	notes = std::vector<Note>(NOTES->GetNotes("notes_bpm180"));
...
void BPM180::Update()
{
	...
	{
		SERIAL->Recv(8, buffer, 1);
		std::cout << "受信されています" << std::endl;
	}
	...
	else
	{
		...
		if (spendTime >= notes[notes.size() - 1].judgeTime + 2)
		{
			...
			SCENE->ChangeScene<BPM180, ResultScene>(result, true);
		}
	}
}

void BPM180::Draw() const
{
	IMAGE->Draw("sunny", Vec2D{ Define::WIN_W - ((int)cnt % Define::WIN_W) , 0 });
	IMAGE->Draw("sunny", Vec2D{ Define::WIN_W - ((int)cnt % Define::WIN_W) - Define::WIN_W , 0 });
