...
JUDGE_TYPE HaikaiScene::Judge(int itr, float spendTime)
{
	//v4.3 added
...
HaikaiScene::HaikaiScene()
{
	notes = std::vector<Note>(NOTES->GetNotes("notes_haikai"));
...
void HaikaiScene::Update()
{
	...
	if (spendTime <= notes[notes.size() - 1].judgeTime + 1)
	...
	else
	{
		...
		if (spendTime >= notes[notes.size() - 1].judgeTime + 2)
		{
			...
			SCENE->ChangeScene<HaikaiScene, ResultScene>(result, true);
		}
	}
}

void HaikaiScene::Draw() const
{
